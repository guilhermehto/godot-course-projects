...
func _add_new_trunk():
	var trunk = TRUNK.instance()
	add_child(trunk)
	trunk.position.y = trunks[trunks.size()-1].position.y - 256
	var left_axe = false if last_has_right_axe else _get_random_chance()
	var right_axe = false if last_has_left_axe else _get_random_chance() and !left_axe
	last_has_left_axe = left_axe
	last_has_right_axe = right_axe
	trunk.initialize_trunk(left_axe, right_axe)
	trunks.append(trunk)
